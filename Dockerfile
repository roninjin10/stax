...
FROM nginx:stable-alpine as example-ui-runner
COPY --from=monorepo /monorepo/packages/example-ui/dist /usr/share/nginx/html
COPY packages/example-ui/nginx.conf /etc/nginx/conf.d/default.conf
EXPOSE 80
